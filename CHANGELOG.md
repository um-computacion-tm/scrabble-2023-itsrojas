...
## Added
- readed the 'place_word' method to 'scrabble_board.py' so it will put the words on the board and it can finally work with the new code

## Fixed
- Finally fix the problem of "validate_word_place_word" tha didn`t work with the correct implementation or the non-empty board. Now it can finally check well if the word began in the center of the table and see if the are another tha crosss the word.
- Fixed "validate_word_inside_word" so it can view if the word that is going to be placed is below or above the board and now it doesn`t make error if the board is not empty.

## Notes of Interest
- 'scrabble_players.py' errors on has_words has been discovered. the proble is that it doesn`t keep the tiles of the player correctly. soon to be fixed
